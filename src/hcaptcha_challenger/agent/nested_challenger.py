...
class NestedAgentConfig(BaseSettings):
    ...
    MAX_IFRAME_SEARCH_DEPTH: int = Field(
        default=4,
        description="The maximum depth of iframe search",
    )
    NESTED_IFRAME_WAIT_TIMEOUT: int = Field(
        default=1000,
        description="The timeout for waiting for the nested iframe to load",
    )
...
class NestedRoboticArm:

    def __init__(self, page: Page, config: NestedAgentConfig):
        self.page = page
        self.config = config
...
class NestedAgentV:
    ...
    def __init__(self, page: Page, agent_config: NestedAgentConfig):
        self.page = page
        self.config = agent_config

        self.robotic_arm = NestedRoboticArm(page=page, config=agent_config)
