...
from hcaptcha_challenger import NestedAgentV, NestedAgentConfig, CaptchaResponse


async def challenge(page: Page) -> NestedAgentV:
    """Automates the process of solving an hCaptcha challenge."""
    # [IMPORTANT] Initialize the Agent before triggering hCaptcha
    agent_config = NestedAgentConfig()
    agent = NestedAgentV(page=page, agent_config=agent_config)
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.goto('file:///Users/jerrymoo/workspace/fooyou/ai/auto-reg-bak/page.html')
        # await page.goto(SiteKey.as_site_link(SiteKey.discord))

        # --- When you encounter hCaptcha in your workflow ---
        agent: NestedAgentV = await challenge(page)
        if agent.cr_list:
